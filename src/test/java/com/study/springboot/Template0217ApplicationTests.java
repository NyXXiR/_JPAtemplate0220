...
class Template0217ApplicationTests {
	...
	public void insertMovie() {
		...
		/*위랑 아래랑 똑같은 얘기인데 builder패턴을 쓴건지, builder패턴은 new를 했음에도 arrayList 객체생성해주지 않는다 
		getter setter 를 쓰면 배열의 빈값으로 나온다. 객체생성을 해준다. */
		Movie movie = new Movie();
		movie.setTitle("영화제목1");
		log.info(movie);
		
		/* poster 부분 posterRepository를 만들지않으면 insert가 되지않아서 
		 * movieRepository에 옵션으로 cascade =CascadeType.ALL
		 * */
		
		Poster poster = Poster.builder().idx(movie.getPosterList().size())
				.name("영화1포스터").movie(movie).build();
		log.info(poster);
		
		Poster poster2 = Poster.builder().idx(movie.getPosterList().size())
				.name("영화2포스터").movie(movie).build();
		log.info(poster2);
	...
	public void insertMasterSalve() {
		...
		master.addSlave(
				Slave.builder().title("연습2").idx(2)
				.content("이것은연습2").build());
		master.addSlave(
				Slave.builder().title("연습3").idx(3)
				.content("이것은연습3").build());
	...
	public void selectMasterSlave() {
		Master master = masterRepository.findById(3L)
						.orElseThrow();	 	//3번이 안나오면 던진다.
	...
	public void testInsProduct() {
		Product product = Product.builder().name("책3").price(50000)
				.stock(3).build();
		
		ProductDetail pd = ProductDetail.builder()
				.content("아주 좋은책").build();
